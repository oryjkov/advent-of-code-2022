...
mod test {
    ...
    fn test_part1() {
        ...
        assert_eq!(solve_part1("input.txt", 10), 4725496);
    }
    #[test]
    fn test_part2() {}
}
...
fn to_iset(inp: &Vec<SB>, ml: i32) -> IntervalSet<i32> {
    inp.iter()
        .map(|sb| {
...
fn calc(inp: &Vec<SB>, ml: i32) -> i32 {
    let i = to_iset(inp, ml);
    let out = inp
        .iter()
        .filter(|sb| sb.beacon.1 == ml)
        .map(|sb| IntervalSet::singleton(sb.beacon.0))
        .fold(IntervalSet::empty(), |set, interval| set.union(&interval));
    i.difference(&out).size() as i32
}
